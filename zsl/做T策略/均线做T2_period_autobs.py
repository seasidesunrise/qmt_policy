...
def handlebar(ContextInfo):
    ...
    for index, row in all_df.iterrows():
        ...
        止损均线 = get_num_by_numfield(row, '做t止损均线')
        ...
        if 做t均线 <= 1:
            ...
            continue
        if 止损均线 <= 1:
            log_and_send_im(f"{策略名称} {table_t} {qmt_code}[{name}] 止损均线设置错误， 止损均线：{止损均线}，请检查，此条做T策略忽略！！")
            continue

        df = get_quatation_by_params(ContextInfo, qmt_code, period, 做t均线, 止损均线)
        curr_data = df.iloc[-1]
        当前价格 = curr_data['close']
        where_clause = " WHERE qmt_code='" + qmt_code + "' AND account_nick='" + cst.account_nick + "'"

        if curr_data['pre_close'] < curr_data['ma' + str(止损均线)]:  # 止损
            ...
            if (相比均线涨幅 >= 高于均线百分比卖出 > 0) and (rt_当前做t状态 == '' or rt_当前做t状态 == '已买回'):  # 做T动作：卖出
                ...
                update_sql = "UPDATE " + table_t + " SET 当前做t状态='" + t_status + "', rt_当前做t资金='" + str(rt_当前做t资金) + "', rt_买回价格='" + str(rt_买回价格) + "', 当前持股数='" + str(0) + "' " + where_clause
